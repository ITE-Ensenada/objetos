...
ScreenAl = 1100
ScreenAn = 600 
SCREEN = SCREEN = pygame.display.set_mode((ScreenAl,ScreenAn))

#Nombre y icono del juego
...
class Nave(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if key[pygame.K_LEFT] and self.rect.left > 0:
			self.rect.x -= VeloNave
		if key[pygame.K_RIGHT] and self.rect.right < ScreenAl and self.rect.right < 500 :
			self.rect.x += VeloNave	
		if key[pygame.K_UP] and self.rect.y > 10 :
			self.rect.y -= VeloNave
		...
		if self.vida_restante > 0:
			pygame.draw.rect(SCREEN,Verde,(self.rect.x,(self.rect.bottom + 10),int(self.rect.width * (self.vida_restante / self.vida_inicio)),15))
		if self.vida_restante == 0:
			self.kill()
		...
		if tiempo < Limite1:
			if tiempo > Limite2/2:
				...
				if tiempo > Limite2:
					...
					if key[pygame.K_n] and tiempo - self.ultima_bala > Ya:
...
class Nave2(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if key[pygame.K_a] and self.rect.left > 0 and self.rect.left > 580:
			self.rect.x -= VeloNave
		if key[pygame.K_d] and self.rect.right < ScreenAl:
			self.rect.x += VeloNave	
		...
		if self.vida_restante >= 1:
			wow = pygame.draw.rect(SCREEN,Verde,(self.rect.x,(self.rect.bottom + 10),int(self.rect.width * (self.vida_restante / self.vida_inicio)),15))
		if self.vida_restante == 0:
			self.kill()
		...
		if tiempo < Limite1:
			if tiempo > Limite2/2:
				...
				if tiempo > Limite2:
					...
					if key[pygame.K_e] and tiempo - self.ultima_bala > Ya:
