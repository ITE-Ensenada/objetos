...
def main():
    ...
    NEW_SURF = BASICFONT.render("NUEVO JUEGO", True, WHITE)
    NEW_RECT = NEW_SURF.get_rect()
    NEW_RECT.topleft = (WINDOWWIDTH - 200, WINDOWHEIGHT - 200)
 
    # The 'Shots:' label at the top
    SHOTS_SURF = BASICFONT.render("TIROS: ", True, WHITE)
...
def check_for_win(board, revealed):
    """
    La función comprueba si el estado actual del tablero es un estado ganador.
    
    tablero: > el tablero que contiene las piezas del barco
    Revelado -> Lista de mosaicos revelados
    devuelve True si se revelan todas las naves
    """
...
def draw_board(board, revealed):
    """
    La función dibuja el tablero de juego.
    
    Board -> Lista de mosaicos de board
    Revelado -> Lista de mosaicos revelados
    """
...
def set_markers(board):
    """
    La función crea las listas de los marcadores al lado del tablero de juego que indica
    El número de piezas de barco en cada fila y columna.
    
    tablero: lista de mosaicos de tablero
    Devuelve las 2 listas de marcadores con el número de piezas de barco en cada fila (xmarkers)
    y columna (ymarkers)
    """
...
def show_help_screen():
    ...
    """
    line1_surf, line1_rect = make_text_objs('Presiona una tecla A para volver al juego', 
                                            BASICFONT, TEXTCOLOR)
    line1_rect.topleft = (TEXT_LEFT_POSN, TEXT_HEIGHT)
    DISPLAYSURF.blit(line1_surf, line1_rect)
    
    line2_surf, line2_rect = make_text_objs(
        '"Este es un juego de rompecabezas de acorazado. Su objetivo es ' \
        'para hundir todos los barcos en tan pocos', BASICFONT, TEXTCOLOR)
    line2_rect.topleft = (TEXT_LEFT_POSN, TEXT_HEIGHT * 3)
    DISPLAYSURF.blit(line2_surf, line2_rect)

    line3_surf, line3_rect = make_text_objs('disparos como sea posible. Los marcadores en'\
        ' los bordes del tablero de juego te dicen cómo', BASICFONT, TEXTCOLOR)
    line3_rect.topleft = (TEXT_LEFT_POSN, TEXT_HEIGHT * 4)
    DISPLAYSURF.blit(line3_surf, line3_rect)

    line4_surf, line4_rect = make_text_objs('Hay muchas piezas de barco en cada uno'\
        ' columna y fila. Para restablecer tu juego, haz clic en', BASICFONT, TEXTCOLOR)
    line4_rect.topleft = (TEXT_LEFT_POSN, TEXT_HEIGHT * 5)
    DISPLAYSURF.blit(line4_surf, line4_rect)

    line5_surf, line5_rect = make_text_objs('el botón "Nuevo juego".',
        BASICFONT, TEXTCOLOR)
...
def show_gameover_screen(shots_fired):
    """
    La función muestra una pantalla de gameover cuando el usuario ha disparado con éxito a cada pieza del barco.
    
    shots_fired -> el número de tiros realizados antes de que termine el juego
    """
    DISPLAYSURF.fill(BGCOLOR)
    titleSurf, titleRect = make_text_objs('¡Felicidades! Puzzle resuelto en:',
    ...
    pressKeySurf, pressKeyRect = make_text_objs(
        'Presiona una tecla ENTER para tratar de superar esa puntuación.', BASICFONT, TEXTCOLOR)
